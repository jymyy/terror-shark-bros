...
function startScreen() {
	...
	ctx.fillText("'S' - Start, 'T' - Setup, 'H' - Help", WIDTH/2, HEIGHT/2 + 100);
...
function helpScreen() {
	...
	ctx.fillText("Mute/Unmute sounds", WIDTH/2 + 150, HEIGHT * 0.60);
	
	ctx.fillText("ESC", WIDTH/2 - 170, HEIGHT * 0.70);
	ctx.fillText("Back/Pause", WIDTH/2 + 150, HEIGHT * 0.70);
	
	ctx.fillText("HINTS", WIDTH/2, HEIGHT * 0.75);
	ctx.fillText("Jump on the sharks to kill them.", WIDTH/2, HEIGHT * 0.80);
	ctx.fillText("You can climb over tall objects by pressing arrow towards it and using jump.", WIDTH/2, HEIGHT * 0.85);
	ctx.fillText("You can earn more lives by collecting stars and 'bögös'.", WIDTH/2, HEIGHT * 0.90);
	ctx.fillText("Sometimes going backwards or to the stars reveals what can't be seen.", WIDTH/2, HEIGHT * 0.95);
}
